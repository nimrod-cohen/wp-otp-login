  ...
  function send_otp() {
    try {
      ...
      if ($emailEnabled && !empty($identifier) && filter_var($identifier, FILTER_VALIDATE_EMAIL)) {
        ...
        WPOTPLogin::log("found user $userId by phone: $phone");
      }
      ...
      WPOTPLogin::log("OTP for user $userId is $code");
      ...
      if ($emailEnabled) {
        ...
        WPOTPLogin::log("email response was: " . json_encode($response));
      }
      if ($phoneEnabled && !empty($phone)) {
        $response = $this->send_otp_phone($phone, $code);
        WPOTPLogin::log("sms response was: " . json_encode($response));
      }
